...
function openEditor(idx){{
  ...
  items.forEach(function(it,i){{
    ...
    rows+="<div class='ed-grid' id='row-"+idx+"-"+i+"'>"
      ...
      +ppCell
      +"<input class='ed-inp' id='p-"+idx+"-"+i+"' type='text' value='$"+Math.round(it.precio_real||0).toLocaleString(\"de-DE\")+"' style='text-align:right;' onfocus=\"this.value=this.value.replace(/[$\.]/g,'')\" onblur=\"var v=parseInt(this.value.replace(/[^0-9]/g,''))||0;this.value='$'+v.toLocaleString('de-DE')\" />"\n      +"<button class='ed-rm' onclick='toggleRm("+idx+","+i+")'>🗑</button>"
      +"</div>";
  }});
...
function saveEdit(idx){{
  ...
  items.forEach(function(it,i){{
    ...
    var p=parseInt((document.getElementById("p-"+idx+"-"+i).value||"").replace(/[^0-9]/g,""))||0;
