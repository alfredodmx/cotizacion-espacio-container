...
def build_catalogo_html(cat_items, supa_url, supa_key, tipo='imagen', cantidad=4):
    ...
    cat_html = ''
    for cat, items in sorted(grupos.items()):
        # Header con botones editar y eliminar categoría
        cat_html += '<div style="background:#1e3a5f;color:white;font-weight:900;padding:7px 12px;border-radius:7px;margin:12px 0 6px;display:flex;justify-content:space-between;align-items:center;">'
        cat_html += '<span>' + cat + '</span>'
        cat_html += '<div style="display:flex;gap:6px;align-items:center;">'
        cat_html += '<span style="font-size:11px;font-weight:400;opacity:0.7;">' + str(len(items)) + ' items</span>'
        ...
        cat_html += '</div></div>'
        # Panel edición (oculto por defecto)
        ids_json = '[' + ','.join(['"' + str(it['id']) + '"' for it in items]) + ']'
        urls_json = '[' + ','.join(['"' + (it.get('imagen_url') or '') + '"' for it in items]) + ']'
        ...
        cat_html += '<div style="font-weight:700;font-size:12px;color:#1e3a5f;margin-bottom:8px;">Editando categoría: ' + cat + '</div>'
        cat_html += '<div style="display:flex;gap:8px;margin-bottom:8px;align-items:center;">'
        cat_html += '<input id="rename-' + cat + '" type="text" value="' + cat + '" style="flex:1;padding:6px 9px;border:1px solid #cbd5e1;border-radius:5px;font-size:12px;" placeholder="Nuevo nombre">'
        ...
        cat_html += '</div>'
        cat_html += '<div style="font-weight:700;font-size:11px;color:#64748b;margin-bottom:6px;">AGREGAR NUEVO ITEM A ESTA CATEGORÍA:</div>'
        cat_html += '<div style="display:flex;gap:8px;margin-bottom:8px;align-items:center;">'
        cat_html += '<input id="new-nombre-' + cat + '" type="text" placeholder="Nombre del item" style="flex:1;padding:6px 9px;border:1px solid #cbd5e1;border-radius:5px;font-size:12px;">'
        cat_html += '<input id="new-file-' + cat + '" type="file" accept="image/*" style="font-size:11px;" onchange="window.previewNewItem(\'' + cat + '\')">'
        cat_html += '<input id="new-hex-' + cat + '" type="color" value="#ffffff" style="width:40px;height:34px;border-radius:5px;border:1px solid #cbd5e1;cursor:pointer;">'
        cat_html += '<button onclick="window.agregarItem(\'' + cat + '\')" style="background:#16a34a;color:white;border:none;border-radius:5px;padding:6px 12px;font-size:12px;cursor:pointer;">+ Agregar</button>'
        cat_html += '</div>'
        cat_html += '<div id="new-prev-' + cat + '" style="margin-bottom:8px;"></div>'
        cat_html += '<div id="edit-status-' + cat + '" style="font-size:11px;font-weight:600;min-height:16px;"></div>'
        cat_html += '</div>'
        # Grid de items
        ...
        for it in items:
            ...
            if iurl:
                img = '<img src="' + iurl + '" style="width:100%;height:80px;object-fit:cover;display:block;">'
            elif it.get('hex'):
            ...
            cat_html += '<button onclick="window.catEliminar(\'' + iid + '\',\'' + iurl + '\')" style="position:absolute;top:3px;right:3px;background:rgba(220,38,38,0.85);color:white;border:none;border-radius:3px;width:20px;height:20px;cursor:pointer;">x</button>'
            cat_html += '</div>'
        cat_html += '</div>'
    ...
    js_lines = [
        ...
        '  if(!confirm("Eliminar toda la categoria "+cat+" y sus "+document.querySelectorAll("#edit-"+cat+" ~ div img").length+" items?"))return;',
        ...
        '    if(url){',
        '      var path=url.split("/public/formulario-imagenes/")[1];',
        '      if(path)await fetch(S+"/storage/v1/object/formulario-imagenes/"+path,',
        '        {method:"DELETE",headers:{"Authorization":"Bearer "+K,"apikey":K}});',
        '    }',
        '  }',
        ...
        # agregar item a categoria existente
        'window.previewNewItem=function(cat){',
        '  var f=document.getElementById("new-file-"+cat).files[0];',
        '  if(!f)return;',
        '  var r=new FileReader();',
        '  r.onload=function(e){document.getElementById("new-prev-"+cat).innerHTML=\'<img src="\'+e.target.result+\'" style="width:60px;height:60px;object-fit:cover;border-radius:5px;">\';};',
        '  r.readAsDataURL(f);',
        '};',
        'window.agregarItem=async function(cat){',
        '  var nombre=document.getElementById("new-nombre-"+cat).value.trim();',
        '  var st=document.getElementById("edit-status-"+cat);',
        '  if(!nombre){st.textContent="Escribe el nombre del item";st.style.color="#dc2626";return;}',
        '  var body={categoria:cat,nombre:nombre,imagen_url:"",activo:true};',
        ...
        '  } else {',
        '    var hex=document.getElementById("new-hex-"+cat).value;',
        '    if(hex&&hex!="#ffffff")body.hex=hex;',
        '  }',
        '  st.textContent="Guardando...";st.style.color="#2563eb";',
        ...
        # preview imagen nueva categoria
...
def leer_datos_actuales():
    ...
    if mejor_tel_asesor is not None:
        raw = re.sub(r'[^0-9]', '', mejor_tel_asesor)
        # Quitar prefijo 56 si viene incluido antes de truncar
...
<style>
    ...
    .resultados-table th:first-child {
        position: sticky !important; left: -2px !important; top: -1px !important; z-index: 3 !important;
        background: linear-gradient(135deg, #1e2447 0%, #2a3060 100%) !important;
        padding-left: 14px !important;
        transform: translateZ(0) !important;
        will-change: transform !important;
...
if tab2 is not None:
 with tab2:
    ...
    # Mostrar heartbeat solo cuando hay carrito activo y no es solo lectura
 ...
 with tab3:
    ...
    <div class="hdr3" style="display:flex!important;align-items:center!important;">
      ...
      <div style="margin-left:16px;">
        ...
        <div style="font-family:Montserrat,sans-serif;font-weight:300;font-size:0.92rem;color:rgba(255,255,255,0.65);margin-top:2px;line-height:1.2;">Busca, carga y administra todas las cotizaciones del sistema.</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.resultados_busqueda:
        ...
        for _, row in df_resultados.iterrows():
            ...
            # Demora — congelar si está rechazado
...
if tab4 is not None:
 with tab4:
    ...
    <div class="hdr4" style="display:flex!important;align-items:center!important;">
      <span style="font-size:2.8rem;line-height:1;flex-shrink:0;">🧊</span>
      <div style="margin-left:16px;">
        <div style="font-family:Montserrat,sans-serif;font-weight:900;font-size:1.6rem;letter-spacing:0.05em;text-transform:uppercase;color:white;line-height:1.1;">Visor 3D Beta</div>
        <div style="font-family:Montserrat,sans-serif;font-weight:300;font-size:0.92rem;color:rgba(255,255,255,0.65);margin-top:2px;line-height:1.2;">Selecciona un presupuesto con plano adjunto para generar su prototipo 3D interactivo.</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.modo_admin and tab5 is not None:
    with tab5:
        ...
        .excel-header {
            ...
            border-radius: 20px; padding: 34px 36px; margin-bottom: 28px; margin-top: -1rem;
            display: flex; align-items: center; gap: 16px;
        ...
        .excel-header h2 { color: #fff !important; margin: 0; font-size: 0.88rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em;
...
if tab_admindata is not None:
    ...
    <div class="hdr-admindata" style="display:flex!important;align-items:center!important;">
      ...
      <div style="margin-left:16px;">
        ...
        <div style="font-family:Montserrat,sans-serif;font-weight:300;font-size:0.92rem;color:rgba(255,255,255,0.65);margin-top:2px;line-height:1.2;">Eliminación permanente de presupuestos y archivos · Solo disponible para root</div>
      </div>
    </div>
        """, unsafe_allow_html=True)
 ...
 with tab_reporte:
    ...
    <div class="hdr-reporte" style="display:flex!important;align-items:center!important;">
      ...
      <div style="margin-left:16px;">
        ...
        <div style="font-family:Montserrat,sans-serif;font-weight:300;font-size:0.92rem;color:rgba(255,255,255,0.65);margin-top:2px;line-height:1.2;">Espacio Container House SpA · Período: {_rep_periodo} · Solo admin y root</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not _data_rep:
        ...
        _fmt_m         = lambda v: f"${v/1_000_000:.1f}M" if v >= 1_000_000 else f"${v/1_000:.0f}K"
        _fmt_clp       = lambda v: "${:,.0f}".format(v).replace(",",".")

        # Tiempo promedio cierre (días entre creación y modificación en autorizadas)
        _tiempos = []
        for r in _autorizadas:
            try:
                _fc = _dt.fromisoformat(r['fecha_creacion'][:10])
                _fm = _dt.fromisoformat(r['fecha_modificacion'][:10])
                _dias = (_fm - _fc).days
                if 0 <= _dias <= 365:
                    _tiempos.append(_dias)
            except Exception:
                pass
        _prom_dias = round(sum(_tiempos)/_len_t, 1) if (_len_t := len(_tiempos)) > 0 else 0
        ...
        # Género inferido desde contrato_datos
        _masc = 0; _fem = 0
        for r in _data_rep:
            try:
        ...
        _monto_nat  = sum(r.get('total_total') or 0 for r in _data_rep if (r.get('cliente_tipo') or '') == 'natural' and 'autorizado' in (r.get('estado','') or '').lower())
        _monto_jur  = sum(r.get('total_total') or 0 for r in _data_rep if (r.get('cliente_tipo') or '') == 'juridica' and 'autorizado' in (r.get('estado','') or '').lower())
        _tick_nat   = round(_monto_nat / max(1, sum(1 for r in _autorizadas if (r.get('cliente_tipo') or '') == 'natural')))
        _tick_jur   = round(_monto_jur / max(1, sum(1 for r in _autorizadas if (r.get('cliente_tipo') or '') == 'juridica')))
        ...
        with _k1:
            st.markdown(_kpi_style.format(label="Total cotizaciones", val=_total_cots,
                sub=f"{_n_aut} autorizadas · {len(_borradores)} borradores"), unsafe_allow_html=True)
        with _k2:
            st.markdown(_kpi_style.format(label="Monto total autorizado", val=_fmt_m(_monto_total),
                sub=f"Ticket prom: {_fmt_m(_monto_total/max(1,_n_aut))}"), unsafe_allow_html=True)
        with _k3:
            st.markdown(_kpi_style.format(label="Tasa de conversión", val=f"{_tasa_conv}%",
                sub=f"{_n_aut} de {_total_cots} cotizaciones"), unsafe_allow_html=True)
        with _k4:
            st.markdown(_kpi_style.format(label="Tiempo promedio cierre", val=f"{_prom_dias} días",
                sub=f"Mín {_min_dias}d · Máx {_max_dias}d"), unsafe_allow_html=True)

        st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)

        # ── Fila 2: Ejecutivos + Género ──
        ...
        # ── Fila 3: Estados + Tipo cliente + Tiempos ──
        ...
        # Ejecutivo con más cotizaciones pero menor conversión
        if _ej_sorted:
        ...
        # Cotizaciones sin autorizar
        if _sin_aut_30 > 0:
            _insights.append(f"<b>{_sin_aut_30} cotizaciones</b> llevan más de 30 días sin autorizar. Revisión urgente recomendada para no perder oportunidades.")

        # Jurídica vs natural
        if _tick_jur > _tick_nat * 1.2 and _n_juridica < _n_natural * 0.3:
            _insights.append(f"El ticket promedio de clientes jurídicos ({_fmt_m(_tick_jur)}) es significativamente mayor que el de personas naturales ({_fmt_m(_tick_nat)}). Aumentar foco en empresas puede mejorar rentabilidad.")
        ...
        if _pct_masc > 70 or _pct_fem > 70:
            ...
            _insights.append(f"El <b>{_pct_dom}% de los clientes son de género {_gen_dom}</b> (inferido desde tratamiento del contrato). Considerar estrategias de marketing diferenciadas.")
...
if tab_oper is not None and _rol_actual in ('root', 'admin', 'operacion'):
 with tab_oper:
    ...
    <div class="hdr-oper" style="display:flex!important;align-items:center!important;">
      <span style="font-size:2.8rem;line-height:1;flex-shrink:0;">⚙️</span>
      <div style="margin-left:16px;">
        ...
        <div style="font-family:Montserrat,sans-serif;font-weight:300;font-size:0.92rem;color:rgba(255,255,255,0.65);margin-top:2px;line-height:1.2;">PDF de compras · planos · seguimiento de fabricación y fidelización de clientes</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
window.addEventListener("message",function(e){{
  if(e.data&&e.data.type==="hist_filtro"){{
    ...
    renderBadges();renderRegs();
                    ...
                    _b_colors = {
                        'normal':      ('#dcfce7','#15803d','#166534'),
                        'con':         ('#ffedd5','#c2410c','#9a3412'),
                        'sin':         ('#f3f4f6','#374151','#1f2937'),
                        'normal_con':  ('#fef9c3','#854d0e','#713f12'),
                        'normal_sin':  ('#f0fdf4','#15803d','#166534'),
                        'con_sin':     ('#fff7ed','#c2410c','#9a3412'),
                        'normal_con_sin': ('#ede9fe','#6d28d9','#4c1d95'),
                    }
                    _b_html = '<style>body{margin:0;padding:0;font-family:sans-serif;overflow:hidden;}</style><div style="display:flex;flex-wrap:wrap;gap:6px;padding:4px 0;">'
                    for _bk2, _bl2 in _b_items:
                        _bc = _b_colors.get(_bk2, ('#f1f5f9','#0f172a','#0f172a'))
                        _b_html += (f'<span id="b-{_bk2}" onclick="sendFiltro(this,\'{_bk2}\')" '
                            f'data-bg="{_bc[0]}" data-col="{_bc[1]}" data-bgact="{_bc[2]}" '
                            f'style="border-radius:99px;padding:5px 14px;font-size:12px;font-weight:700;cursor:pointer;border:1.5px solid {_bc[2]}33;background:{_bc[0]};color:{_bc[1]};">{_bl2}</span>')
                    _b_html += ('</div><script>var _f="";'
                        'function sendFiltro(el,v){'
                        '_f=_f===v?"":v;'
                        'document.querySelectorAll("span[id]").forEach(function(s){'
                        'var act=_f&&s.id==="b-"+_f;'
                        's.style.background=act?s.dataset.bgact:s.dataset.bg;'
                        's.style.color=act?"#fff":s.dataset.col;'
                        's.style.boxShadow=act?"0 0 0 2px "+s.dataset.bgact:"none";});'
                        'var iframes=window.parent.document.querySelectorAll("iframe");'
                        'iframes.forEach(function(fr){try{fr.contentWindow.postMessage({type:"hist_filtro",val:_f},"*");}catch(e){}});'
                        '}</script>')
...
if tab6 is not None:
    ...
    <div class="hdr6" style="display:flex!important;align-items:center!important;">
      ...
      <div style="margin-left:16px;">
        ...
        <div style="font-family:Montserrat,sans-serif;font-weight:300;font-size:0.92rem;color:rgba(255,255,255,0.65);margin-top:2px;line-height:1.2;">Busca tu cotización por número EP y personaliza la descripción de cada categoría para el cliente.</div>
      </div>
    </div>
        """, unsafe_allow_html=True)
...
if tab_contrato is not None:
 with tab_contrato:
    ...
    <div class="hdr-contrato" style="display:flex!important;align-items:center!important;">
      <span style="font-size:2.8rem;line-height:1;flex-shrink:0;">📄</span>
      <div style="margin-left:16px;">
        <div style="font-family:Montserrat,sans-serif;font-weight:900;font-size:1.6rem;letter-spacing:0.05em;text-transform:uppercase;color:white;line-height:1.1;">Contrato Cliente</div>
        <div style="font-family:Montserrat,sans-serif;font-weight:300;font-size:0.92rem;color:rgba(255,255,255,0.65);margin-top:2px;line-height:1.2;">Genera y administra el contrato de fabricación y venta.</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
            ...
            if not _cot_pv.get("contrato_generado"):
                ...
                with st.spinner("Generando vista previa..."):
                    try:
                        ...
                        # ── 5. Subir PDF combinado a Supabase Storage y mostrar visor ──
                        import streamlit.components.v1 as _comp_pv
                        import urllib.parse as _urlparse_pv

                        # Subir a storage temporalmente
                        ...
                        if _prev_url:
                            _nav_pv = detectar_navegador()
                            _prev_url_enc = _urlparse_pv.quote(_prev_url, safe='')
                            _google_url_pv = f"https://docs.google.com/viewer?url={_prev_url_enc}&embedded=true"
                            _usar_google_pv = _nav_pv['needs_google_viewer']
                            _src_pv = _google_url_pv if _usar_google_pv else _prev_url
...
<div id="pdf-wrap">
  <div id="pdf-loading">
    <div id="pdf-spinner"></div>
    <span id="pdf-status">Cargando contrato completo...</span>
  </div>
  <iframe id="pdf-iframe" src="" allow="fullscreen"></iframe>
</div>
<script>
(function() {{
  var iframe  = document.getElementById('pdf-iframe');
  var loading = document.getElementById('pdf-loading');
  var googleUrl = "{_google_url_pv}";
  var directUrl = "{_prev_url}";
  var usingGoogle = {"true" if _usar_google_pv else "false"};
  function hideLoading() {{
    loading.style.opacity = '0';
    setTimeout(function(){{ loading.style.display = 'none'; }}, 400);
  }}
...
""", height=710, scrolling=False)
                        ...
                        with _dc2:
                            _nom_dl = f"ContratoCompleto_{_ep_pv_num.replace('-','_')}.pdf"
                            st.download_button(
...
if st.session_state.modo_admin and tab_usuarios is not None:
    ...
    <div class="hdr-usuarios" style="display:flex!important;align-items:center!important;">
      ...
      <div style="margin-left:16px;">
        ...
        <div style="font-family:Montserrat,sans-serif;font-weight:300;font-size:0.92rem;color:rgba(255,255,255,0.65);margin-top:2px;line-height:1.2;">Crea y administra las cuentas de acceso de los ejecutivos.</div>
      </div>
    </div>
        """, unsafe_allow_html=True)
