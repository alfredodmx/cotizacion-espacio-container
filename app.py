...
def generar_pdf_contrato(datos, clausulas_externas=None):
    ...
    _ORIG = {
        ...
        "comparecencia_cliente": f"{d.get('cli_tratamiento','Don')} <b>{d.get('cli_nombre','')}</b>, cédula nacional de identidad N° <b>{d.get('cli_rut','')}</b>, con domicilio en <b>{d.get('cli_domicilio','')}</b>, comuna de <b>{d.get('cli_comuna','')}</b>, Región {d.get('cli_region','')}, quien en adelante se denominará \"el Cliente\".\n\nSe deja expresa constancia que la dirección de instalación del proyecto será <b>{d.get('inst_domicilio','')}</b>, comuna de <b>{d.get('inst_comuna','')}</b>, Región {d.get('inst_region','')}.\n\nLas partes declaran ser mayores de edad, con plena capacidad legal para contratar, y acuerdan celebrar el presente <b>Contrato de Fabricación y Venta de Vivienda Tipo Container</b>, el cual se regirá por las cláusulas que se indican a continuación.",
