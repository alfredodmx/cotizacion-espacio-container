...
# FIX #2: Variable para rastrear qué cotización está en el visor
...
# =========================================================
# FIX #1: Función auxiliar para leer datos actuales de los widgets
# Lee los valores directamente del session_state sin depender del counter
# =========================================================
def leer_datos_actuales():
    """
    Sincroniza session_state buscando los widgets dinámicos en TODOS los
    counters posibles (no solo el actual). Esto garantiza que los datos
    se lean correctamente sin importar cuántos reruns hayan ocurrido.
    El widget más reciente (counter más alto) tiene prioridad.
    """
    # Mapeo: prefijo del key → campo en session_state y transformación
    mapeo_texto = {
    ...
    # Para cada campo de texto simple, buscar el key con el counter más alto
    ...
    # RUT: buscar el key más reciente y procesar
    mejor_counter = -1
    mejor_rut = None
    for key, valor in st.session_state.items():
...
def construir_datos_para_guardar():
    ...
    usando siempre session_state como fuente de verdad.
    """
    leer_datos_actuales()
    ...
    plano_datos = base64.b64encode(st.session_state.plano_adjunto).decode() if st.session_state.plano_adjunto else None
...
def init_database():
    ...
    c = conn.cursor()
    c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='cotizaciones'")
    ...
    if not tabla_existe:
        ...
        if 'plano_nombre' not in columnas:
            c.execute("ALTER TABLE cotizaciones ADD COLUMN plano_nombre TEXT")
        if 'plano_datos' not in columnas:
            c.execute("ALTER TABLE cotizaciones ADD COLUMN plano_datos TEXT")
    conn.commit()
    conn.close()
...
def guardar_cotizacion(numero, cliente, asesor, proyecto, productos, config, totales, plano_nombre=None, plano_datos=None):
    ...
    - Si el número ya existe: hace UPDATE preservando fecha_creacion.
    - Si es nueva: hace INSERT con fecha_creacion = ahora.
    Retorna True/False y muestra el error en pantalla si falla.
    """
    conn = None
    try:
        conn = sqlite3.connect('cotizaciones.db', timeout=10)
        conn.isolation_level = None  # autocommit desactivado, control manual
        c = conn.cursor()
        ...
        tiene_plano = plano_nombre is not None and plano_nombre != ''
        datos_completos = all([
        ...
        c.execute("SELECT fecha_creacion FROM cotizaciones WHERE numero = ?", (numero,))
        fila_existente = c.fetchone()
        ...
        campos_valores = {
            ...
            'plano_datos': plano_datos,
        }

        c.execute("BEGIN")

        if fila_existente:
            # UPDATE: preserva fecha_creacion original
            sets = ', '.join(f"{k} = ?" for k in campos_valores)
            valores = list(campos_valores.values()) + [numero]
            c.execute(f"UPDATE cotizaciones SET {sets} WHERE numero = ?", valores)
        else:
            # INSERT nuevo
            campos_valores['numero'] = numero
            campos_valores['fecha_creacion'] = fecha_actual
            cols = ', '.join(campos_valores.keys())
            placeholders = ', '.join('?' for _ in campos_valores)
            c.execute(f"INSERT INTO cotizaciones ({cols}) VALUES ({placeholders})", list(campos_valores.values()))
...
def buscar_cotizaciones(termino=None, tipo_busqueda='numero'):
    ...
    base_select = '''
        SELECT numero, cliente_nombre, asesor_nombre, fecha_creacion, total_total,
               ...
               CASE WHEN plano_nombre IS NOT NULL AND plano_nombre != '' THEN 1 ELSE 0 END as tiene_plano
        FROM cotizaciones
    '''
    if termino and termino.strip():
        ...
        c.execute(f"{base_select} WHERE {campo} LIKE ? ORDER BY fecha_creacion DESC", (f'%{termino}%',))
    else:
        c.execute(f"{base_select} ORDER BY fecha_creacion DESC LIMIT 50")
...
def actualizar_estado_cotizacion(numero, estado):
    ...
    c = conn.cursor()
    fecha_actual = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    c.execute('UPDATE cotizaciones SET estado = ?, fecha_modificacion = ? WHERE numero = ?', (estado, fecha_actual, numero))
    conn.commit()
    conn.close()

# =========================================================
# FIX #4: Generador de número único para evitar colisiones
# =========================================================
def generar_numero_unico():
    """Genera un número de cotización único verificando que no exista en la BD."""
    conn = sqlite3.connect('cotizaciones.db')
    c = conn.cursor()
    intentos = 0
    while intentos < 20:
        numero = f"EP-{random.randint(10000, 99999)}"  # 5 dígitos = 90.000 posibilidades
        c.execute("SELECT 1 FROM cotizaciones WHERE numero = ?", (numero,))
        if not c.fetchone():
            conn.close()
            return numero
        intentos += 1
...
def ejecutar_carga_cotizacion():
    if st.session_state.cargar_cotizacion_trigger and st.session_state.cotizacion_a_cargar:
        ...
        if plano_nombre and plano_datos:
            ...
            st.session_state.plano_adjunto = base64.b64decode(plano_datos)
        else:
            st.session_state.plano_nombre = ""
            st.session_state.plano_adjunto = None
        ...
        # FIX #2: Resetear visor al cargar nueva cotización
        st.session_state.mostrar_visor = False
        st.session_state.pdf_actual = None
        st.session_state.pdf_nombre = ""
        st.session_state.numero_en_visor = None
...
# HEADER
# =========================================================
# Marcador HTML para identificar el header
...
# Header: título a la izquierda, logo+admin flotando a la derecha con position absolute
...
# Botón Admin alineado a la derecha con columnas trucadas
...
if st.session_state.cotizacion_cargada:
    ...
    with col_cerrar:
        # FIX #1: Usar construir_datos_para_guardar() para capturar datos actuales antes de cerrar
        if st.button("🗑️ Cerrar Cotización", key="btn_cerrar_cotizacion", use_container_width=True):
            ...
            st.session_state.pdf_actual = None
            st.session_state.pdf_nombre = ""
            st.session_state.numero_en_visor = None  # FIX #2
            st.session_state.counter += 100
...
def generar_pdf_cliente(carrito_df, subtotal, iva, total, datos_cliente,
    ...
    for categoria, grupo in categorias:
        ...
        # Todos los items, separados por salto de línea, contenidos en Paragraph
...
with tab2:
    ...
    if es_solo_lectura:
        ...
        asesores = {
            ...
            "María Pérez": {"correo": "maria.perez@empresa.cl", "telefono": "987654321"},
            "Juanito Carmona": {"correo": "juan.carmona@empresa.cl", "telefono": "912345678"},
            "Carlos Rodríguez": {"correo": "carlos.rodriguez@empresa.cl", "telefono": "955443322"},
            "Ana Martínez": {"correo": "ana.martinez@empresa.cl", "telefono": "966778899"},
            "Pedro Soto": {"correo": "pedro.soto@empresa.cl", "telefono": "944332211"}
        }
...
with tab1:
    ...
    if not es_solo_lectura:
        ...
        with col_plano:
            # Estilo para que el file_uploader parezca un botón morado
            st.markdown('''
            <style>
        ...
        with col_btn1:
            if not es_solo_lectura:
                if st.button("💾 Guardar", use_container_width=True, type="primary"):
                    # FIX #1: Usar construir_datos_para_guardar() que lee todos los widgets activos
                    datos_cliente_guardar, datos_asesor_guardar, proyecto, config, totales, plano_nombre, plano_datos = construir_datos_para_guardar()
                    ...
                    if st.session_state.cotizacion_cargada:
                        ...
                        # FIX #4: Usar generador de número único
...
with tab3:
    ...
    if buscar_btn or (termino and termino != st.session_state.get('ultimo_termino', '')):
        ...
        # Resetear visor al hacer nueva búsqueda
        st.session_state.mostrar_visor = False
        st.session_state.pdf_actual = None
        st.session_state.pdf_nombre = ""
        st.session_state.numero_en_visor = None
    ...
    if st.session_state.resultados_busqueda:
        ...
        if opciones:
            ...
            if cotizacion_seleccionada:
                ...
                # VISOR: si cambia la selección, actualizar el PDF automáticamente
                # si el visor ya estaba abierto Y la nueva cotización también tiene plano.
                # Si la nueva no tiene plano, cerrar el visor.
                if numero_seleccionado != st.session_state.numero_en_visor:
                    if tiene_plano_seleccionado and st.session_state.mostrar_visor:
                        # Cargar automáticamente el nuevo PDF en el visor
                        cot_visor = cargar_cotizacion(numero_seleccionado)
                        if cot_visor and cot_visor.get('plano_datos'):
                            st.session_state.pdf_actual = base64.b64decode(cot_visor['plano_datos'])
                            st.session_state.pdf_nombre = cot_visor.get('plano_nombre', 'plano.pdf')
                            st.session_state.numero_en_visor = numero_seleccionado
                            # rerun necesario para que el iframe HTML se recree con el nuevo key
                            st.rerun()
                        else:
                            st.session_state.mostrar_visor = False
                            st.session_state.pdf_actual = None
                        ...
                        # La cotización nueva no tiene plano, o el visor estaba cerrado: cerrar visor
            ...
            # Cargar cotización UNA sola vez para ambos PDFs
                ...
                if cotizacion_seleccionada and tiene_plano_seleccionado:
                    ...
                    if st.button(label_visor, use_container_width=True, type="primary"):
                        ...
                        if cot_btn and cot_btn.get('plano_datos'):
                            st.session_state.pdf_actual = base64.b64decode(cot_btn['plano_datos'])
        ...
        for col, title, number, css_class, desc in stats:
            with col:
                # Determinar si css_class es una clase CSS o un estilo inline
                # Reducir fuente si el número es largo (ej: montos)
                if len(number) > 12:
                    font_size = "1.6rem"
                elif len(number) > 8:
                    font_size = "2rem"
